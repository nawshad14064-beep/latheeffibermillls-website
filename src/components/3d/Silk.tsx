...
const vertexShader = `
  ...
  void main() {
    ...
    float noise = pnoise(vec3(position.xy * 0.5, uTime * uSpeed), vec3(10.0));
    vDistortion = noise * uNoiseIntensity;
...
const fragmentShader = `
  ...
  void main() {
    float brightness = 0.5 + 0.5 * vDistortion;
...
function SilkScene({ speed = 3.9, scale = 0.8, color = "#7B7481", noiseIntensity = 1.5, rotation = 0 }: SilkProps) {
  ...
  return (
    <mesh ref={meshRef} scale={[scale * 10, scale * 10, 1]}>
